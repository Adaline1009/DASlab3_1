        ...
        ConListenerInterface
{
    ...
    public DownloadManager(TorrentFile torrent, final byte[] clientID)
    {
        ...
        for (int i = 0; i < this.nbPieces; i++)
        {
            ...
            do
            {
                ...
                if ((torrent.length.size() > file) && fileoffset + this.torrent.pieceLength - pieceoffset >=
    ...
    public synchronized void savePiece(int piece)
    {
        ...
        for (Iterator it = this.pieceList[piece].getFileAndOffset()
                .keySet()
                .
                        iterator(); it.hasNext(); )
        {
            try
            {
                ...
                this.output_files[file.intValue()].seek(((Integer)
                        (this.pieceList[piece].getFileAndOffset()
                                .get(file))).
                        intValue());
        ...
        synchronized (this.isComplete)
        {
            ...
            for (int i = 0; i < this.nbPieces; i++)
            {
                        ...
                        this.peerAvailabilies.get(id) != null)
                {
                    ...
                    if (this.peerAvailabilies.get(id)
                            .get(i))
                    {
                        possible.add(i);
                    }
            ...
            boolean complete)
    {
        ...
        synchronized (this.isComplete)
        {
            if (complete && !this.isPieceComplete(i))
            {
                ...
                for (Iterator it = this.task.keySet()
                        .iterator();
                     it.hasNext(); )
                {
                    try
    ...
    public synchronized void peerReady(String peerID)
    {
        ...
        if (piece2request != -1)
        {
            this.task.get(peerID)
                    .requestPiece(this.pieceList[piece2request]);
        }
    }
    ...
    public synchronized void connectionAccepted(Socket s)
    {
        synchronized (this.task)
        {
            ...
            if (!this.task.containsKey(id))
            {
                ...
                dt.addDTListener(this);
                this.peerList.put(dt.getPeer()
                        .toString(), dt.getPeer());
                this.task.put(dt.getPeer()
                        .toString(), dt);
                dt.start();
            }
        }
